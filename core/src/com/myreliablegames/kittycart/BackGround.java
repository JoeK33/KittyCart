        ...
        for (int i = 0; i < numTilesWide; i++) {
            ...
            for (int j = 0; j < NUMBER_OF_BOTTOM_TILE_ROWS; j++) {
                if (scrollingPoint + (i * tileSize) < Constants.WORLD_WIDTH && -tileSize * j < camera.position.y) {
                    batch.draw(Zone.getBottomTile(), scrollingPoint + (i * tileSize), -tileSize * j);
                }
            }

            for (int j = 0; j < NUMBER_OF_MIDDLE_TILE_ROWS; j++) {
                if (scrollingPoint + (i * tileSize) < Constants.WORLD_WIDTH) {
                    batch.draw(Zone.getMiddleTile(), scrollingPoint + (i * tileSize), (tileSize * j) + tileSize);
                }
            }
            ...
            for (int j = 0; j < NUMBER_OF_TOP_TILE_ROWS; j++) {
                ...
                if (scrollingPoint + (i * tileSize) < Constants.WORLD_WIDTH &&
                        tileSize * (j + NUMBER_OF_MIDDLE_TILE_ROWS * 2) + tileSize < (camera.position.y + camera.viewportHeight)) {
                    batch.draw(Zone.getTopTile(), scrollingPoint + (i * tileSize), tileSize * (j + NUMBER_OF_MIDDLE_TILE_ROWS * 2) + tileSize);
                }
            }
        }
